...
IF DEFINED USE_MSBUILD (
  ...
  IF !ERRORLEVEL! NEQ 0 goto error
  call :ExecuteCmd dotnet publish "D:\home\site\repository\src\Alex" --output "%DEPLOYMENT_TEMP%" --configuration Release --no-build
  IF !ERRORLEVEL! NEQ 0 goto error
) ELSE (
  call :ExecuteCmd dotnet publish "D:\home\site\repository\src\Alex" --output "%DEPLOYMENT_TEMP%" --configuration Release
  IF !ERRORLEVEL! NEQ 0 goto error
)
